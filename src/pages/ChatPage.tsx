...
interface UploadedFile {
  ...
  error?: string;
  /** Unique key so duplicate filenames are distinct */
  id: string;
}
...
const ChatPage = () => {
  ...
  }, [messagesQuery.data, uploadedFiles, optimisticMessages, sendMessageMutation.isPending]);
  ...
  // When server messages arrive, remove the matching optimistic messages
  useEffect(() => {
    if (!messagesQuery.data || optimisticMessages.length === 0) return;
    setOptimisticMessages((prev) =>
      prev.filter(
        (opt) =>
          !messagesQuery.data!.some(
            (msg: any) => msg.role === "USER" && msg.content === opt.content
          )
      )
    );
  }, [messagesQuery.data]);
  ...
  const handleSend = useCallback(
    (e: React.FormEvent) => {
      ...
      sendMessageMutation.mutate(trimmed, {
        onError: () => {
          // Remove optimistic bubble on error so user can retry
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    ingestFileMutation.mutate(
      ...
      {
        onSuccess: () => {
          // Langsung hapus dari state setelah selesai — tidak ditampilkan di bubble
  ...
  // Hanya tampilkan file yang sedang uploading atau error.
  // File yang sudah done langsung dihapus dari state (tidak tampil di bubble).
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-semibold text-sm text-gray-900 leading-tight">
                {roomQuery.data?.name ?? 'AI Study Room'}
              </h2>
          ...
          <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
            ...
            {messagesQuery.data?.map((msg: any) => (
              ...
              >
                ...
                {/* Bubble */}
                <div className={`max-w-[75%] ${msg.role !== "BOT" ? "text-right" : ""}`}>
                  ...
                  {msg.role === "BOT" ? (
                    ...
                    <div className="px-4 py-3 rounded-2xl text-sm leading-relaxed bg-gray-900 text-white">
                      {msg.content}
                    </div>
                  )}
                </div>
            ...
            {/* ✅ Optimistic user bubbles — appears instantly when user sends */}
            <AnimatePresence>
              {optimisticMessages.map((msg) => (
                ...
                >
                  ...
                  <div className="max-w-[75%] text-right">
                    ...
                    <div className="px-4 py-3 rounded-2xl text-sm leading-relaxed bg-gray-900 text-white opacity-80">
                      {msg.content}
                    </div>
                  </div>
                </motion.div>
            ...
            {/* ✅ Uploaded file bubbles — keyed per upload session, NOT persistent */}
            <AnimatePresence>
